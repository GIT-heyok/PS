...
struct segtree{
    ...
    void update(int ss, int se, int ii, int idx ,ll value)
    {
        ...
        t[idx].second = max(t[2*idx+1].second, t[2*idx+2].second);
    }
...
/*
Implemenation is sane with getting min, max value among range
*/
int main() {
    FAST
    int T;
    ...
    while(T--){
        ...
        for (int i = 0; i < q; i++)
        {
            ...
            if(type==1){
                ...
                ll first = s.get(a,a).first;
                ll second = s.get(b,b).first;
                s.update(a,second);
                s.update(b,first);
            }
        }
